...
export default function MyPage() {
  ...
  return <main className="mypage-main mypage-refresh">
    <section className="mypage-profile-card"><div className="mypage-avatar">{avatarUrl ? <img src={avatarUrl} alt={`${displayName} 프로필`} /> : <span className="mypage-avatar-initials">{displayName.slice(0, 1)}</span>}</div><div className="mypage-profile-copy"><span>MY LOCAL LIFE</span><h1>{displayName}님의 전라도 이야기</h1><p className="mypage-profile-description">여행의 추억부터 새로운 일까지, 나의 활동을 한곳에서 확인해요.</p><p>{email || "이메일 정보 없음"}</p></div><div className="mypage-profile-actions"><button onClick={logout}>로그아웃</button></div></section>
