...
const tabs = [["profile", "내 정보"], ["guides", "저장한 여행 가이드"], ["aiMatches", "AI 매칭 결과"], ["posts", "내 여행 공유"], ["applications", "내가 지원한 공고"], ["favoriteJobs", "찜한 일자리"], ["gatherings", "내 게더링"]];
...
function aiMatchResults(match = {}) {
  return Array.isArray(match.results) ? match.results : [];
}

function aiMatchStatusLabel(status) {
  return { COMPLETED: "추천 완료", PROCESSING: "추천 생성 중", REPLACED: "대체 추천", FAILED: "추천 실패" }[String(status || "").toUpperCase()] || status || "상태 확인 중";
}
