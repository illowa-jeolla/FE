...
import { getSavedTravelGuide, getTravelGuideDraft, removeSavedTravelGuide, requestTravelGuideAlternative, requestTravelRecommendation, saveTravelGuideDraft, waitForTravelRecommendation } from "../api/travelRecommendations";
...
function normalizeDraft(detail, conditions) { return (detail.days || []).map((day) => ({ region: detail.regionName || conditions.region || "전라도", hotel: { name: detail.accommodation?.name || conditions.hotel || "추천 출발지", latitude: detail.accommodation?.latitude, longitude: detail.accommodation?.longitude }, tip: detail.travelTip || "여행 전 운영 시간과 이동 방법을 확인해 주세요.", spots: (day.items || []).map((item) => ({ name: item.title || item.name, address: item.address || "", category: item.category || "관광지", description: item.reason || item.description || "추천 관광지입니다.", latitude: item.latitude, longitude: item.longitude, imageUrl: item.thumbnailUrl || "", time: String(item.recommendedTime || "").slice(0, 5), stayMinutes: item.stayMinutes, travelMinutes: item.travelMinutes, distanceFromPreviousKm: item.distanceFromPreviousKm || 0 })) })); }
...
export default function TravelGuidePage() {
  ...
  async function load(alternative = false) {
    ...
    try {
      let response = alternative && generatedDraftId ? await requestTravelGuideAlternative(generatedDraftId) : await requestTravelRecommendation({ ...conditions, attempt: nextAttempt, dailyPlaceCounts: conditions.dailyPlaceCounts || [conditions.placeCount || 3] });
      ...
      setGeneratedDraftId(String(nextDraftId)); setGuides(next); setAttempt(nextAttempt); setSaved(false); setSavedId(""); remember(next, { attempt: nextAttempt, saved: false, savedGuideId: "", draftId: String(nextDraftId) });
    } catch (requestError) {
