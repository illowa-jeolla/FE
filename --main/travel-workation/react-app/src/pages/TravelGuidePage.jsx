...
import { createManualTravelGuide, getNearbyManualPlaces, getSavedTravelGuide, getTourPlaceDetail, getTravelGuideDraft, removeSavedTravelGuide, requestTravelGuideAlternative, requestTravelRecommendation, saveTravelGuideDraft, waitForTravelRecommendation } from "../api/travelRecommendations";
...
export default function TravelGuidePage() {
  ...
  const displayedSpot = spotDetail?.contentId === spot?.contentId ? { ...spot, ...spotDetail } : spot;

  useEffect(() => {
    let current = true;
    setSpotDetail(null);
    setDetailExpanded(false);
    if (!spot?.contentId) return () => { current = false; };
    getTourPlaceDetail(spot.contentId).then((detail) => { if (current) setSpotDetail({ contentId: String(detail.contentId || spot.contentId), name: detail.title || spot.name, address: detail.address || spot.address, description: detail.overview || spot.description, imageUrl: detail.firstImage || detail.firstImageThumbnail || spot.imageUrl, sourceUrl: homepageUrl(detail.homepage) || spot.sourceUrl, sourceTitle: homepageUrl(detail.homepage) ? "공식 사이트에서 보기" : spot.sourceTitle }); }).catch(() => {});
  ...
  return <main className="travel-result-main"><section className="travel-guide-content travel-result-content">
    ...
    <div className="travel-route-layout" key={activeDay}><aside className="travel-place-panel"><div className="travel-route-summary"><span>DAY {activeDay + 1} · 추천 동선</span><b>관광지 {guide.spots.length}곳 · 이동 약 {routeDuration(totalTravelMinutes)} · {totalDistance.toFixed(1)}km</b></div><div>{guide.spots.map((item, index) => <button className={`travel-place-card${!returnMode && index === active ? " is-active" : ""}`} type="button" draggable="true" onDragStart={() => setDragged(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropAt(index)} onClick={() => { setActive(index); setReturnMode(false); }} onContextMenu={(event) => { event.preventDefault(); removeSpot(index); }} title="끌어서 순서 변경 · 우클릭으로 제거" key={`${item.name}-${index}`}><span className="travel-place-number">{index + 1}</span><span><b>{item.name}</b><small>{item.category}</small></span><span className="travel-driving-info"><b>{item.routeEstimated ? "예상 " : "약 "}{routeDuration(item.travelMinutes)}</b><small>{item.distanceFromPreviousKm.toFixed(1)}km</small></span></button>)}</div><button className="travel-add-place" type="button" onClick={() => { setPlaceModal(true); setPlaceResults([]); }}><b>＋</b> 관광지 추가하기</button><button className={`travel-return-hotel${returnMode ? " is-active" : ""}`} type="button" onClick={() => setReturnMode(true)}><span>{guide.destinationLabel}</span><small>{finalLegText}</small></button></aside>
      <section className="travel-route-map travel-kakao-map-wrap" aria-label="숙소 주변 추천 관광지와 여행 경로"><KakaoRouteMap guide={guide} active={returnMode ? guide.spots.length : active} onSelect={(index) => { setActive(index); setReturnMode(false); }} />{!returnMode && displayedSpot && <article className={`travel-map-card${detailExpanded ? " is-expanded" : ""}`}>{displayedSpot.imageUrl && <img className="travel-map-card-image" src={displayedSpot.imageUrl} alt="" />}<div className="travel-map-card-copy"><div className="travel-map-card-heading"><span>{active + 1}</span><b>{displayedSpot.name}</b></div><small>{displayedSpot.address || displayedSpot.category}</small><p>{displayedSpot.description}</p><div className="travel-map-card-actions"><button type="button" onClick={() => setDetailExpanded((value) => !value)}>{detailExpanded ? "접기" : "더보기"}</button>{detailExpanded && displayedSpot.sourceUrl && <a className="travel-map-card-site-link" href={displayedSpot.sourceUrl} target="_blank" rel="noreferrer">사이트 이동 ↗</a>}</div></div></article>}</section>
    </div>
