...
export default function CommunityWritePage() {
  ...
  async function submit(event) { event.preventDefault(); if (!hasSession()) { navigate("/auth"); return; } const values = Object.fromEntries(new FormData(event.currentTarget)); values.imageData = images; values.imagesData = images; try { setError(false); setMessage("여행 기록을 공유하는 중입니다."); const result = await apiRequest("/api/posts", { method: "POST", body: JSON.stringify(values) }); navigate(`/community/${result.id}`); } catch (e) { setError(true); setMessage(e.message); } }
  return <main className="community-write-main">
    ...
    <form className="community-write-card" onSubmit={submit}>
      ...
      <div className="form-row"><label>지역<select name="region" required><option value="">지역 선택</option>{regions.slice(1).map((item) => <option key={item}>{item}</option>)}</select></label><label>여행 콘셉트<input name="concept" placeholder="예: 노을 드라이브" required /></label></div>
